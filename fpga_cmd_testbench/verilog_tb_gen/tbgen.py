...
class TestbenchGenerator(object):
    ...
    def print_module_head(self):
        ...
        self.printo("//the first modul is necessary otherwise quartus wouldn't allow AnalysisSynthesis\n")
        self.printo("//this is literaly a dummy modul\n")
        self.printo("module TB_%s(input clock,input reset);\n" % self.mod_name)
        self.printo("reg foo;\n")
        self.printo("always @(posedge clock)\n")
        self.printo("foo <= 0;\n")
...
if __name__ == "__main__":
    ...
    Based on Author: Xiongfei(Alex) Guo <xfguo@credosemi.com>
        License: Beerware

    Redone by Sausy_ (Roboy)
    optimized for Quartus and personal workflow
